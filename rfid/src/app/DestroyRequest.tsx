    ...
    <View>
      <TouchableOpacity
        style={{
          ...
          borderRadius: 12, paddingHorizontal: 10, height: 38,
        }}
        onPress={() => setOpen(true)}
        activeOpacity={0.7}
      >
        <Text
          style={{ flex: 1, fontSize: 12, fontWeight: '500', color: value ? '#0F172A' : '#94A3B8' }}
          numberOfLines={1}
        >
...
function FilterField({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <View style={{ flex: 1, minWidth: 90 }}>
      <Text style={{ fontSize: 9, fontWeight: '700', color: '#94A3B8', letterSpacing: 1.1, textTransform: 'uppercase', marginBottom: 4 }}>
        {label}
      </Text>
      {children}
    </View>
...
function Checkbox({ value, onToggle, label }: { value: boolean; onToggle: () => void; label?: string }) {
  return (
    ...
    >
      ...
      {label ? <Text style={{ fontSize: 12, fontWeight: '600', color: '#334155' }}>{label}</Text> : null}
    </TouchableOpacity>
  );
}
...
export default function DestroyRequest() {
  ...
  const [destroyRows, setDestroyRows] = useState<boolean[]>(TABLE_DATA.map(r => r.Destroy));

  const tableCols = useMemo(() => calcColWidths(TABLE_COLS_DEF, TABLE_DATA), []);
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#F8FAFC' }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 12, backgroundColor: 'white', borderBottomWidth: 1, borderColor: '#F1F5F9' }}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={{ fontSize: 17, fontWeight: '700', color: '#0F172A' }}>Destroy Request</Text>
      ...
      {/* ── TOOLBAR / FILTER ── */}
      <View style={{ backgroundColor: 'white', borderBottomWidth: 1, borderColor: '#E2E8F0', paddingHorizontal: 12, paddingTop: 10, paddingBottom: 10, gap: 8 }}>

        {/* Row 1: EPC (full width) */}
        <View>
          <Text style={{ fontSize: 9, fontWeight: '700', color: '#94A3B8', letterSpacing: 1.1, textTransform: 'uppercase', marginBottom: 4 }}>EPC</Text>
          <View style={{ height: 38, backgroundColor: 'white', borderWidth: 2, borderColor: '#E2E8F0', borderRadius: 12, paddingHorizontal: 12, justifyContent: 'center' }}>
            <TextInput
              value={epc}
              onChangeText={setEpc}
        ...
        <View style={{ flexDirection: 'row', gap: 8 }}>
          <FilterField label="Model Name">
            <CompactDropdown value={model} options={MODEL_LIST} onSelect={setModel} />
          </FilterField>
          <FilterField label="Stage">
            <CompactDropdown value={stage} options={STAGE_LIST} onSelect={setStage} />
          </FilterField>
          <FilterField label="Season">
            <CompactDropdown value={season} options={SEASON_LIST} onSelect={setSeason} />
          </FilterField>
        </View>
        ...
        <View style={{ flexDirection: 'row', gap: 8 }}>
          <FilterField label="Category">
            <CompactDropdown value={category} options={CATEGORY_LIST} onSelect={setCategory} />
          </FilterField>
          <FilterField label="Article">
            <CompactDropdown value={article} options={ARTICLE_LIST} onSelect={setArticle} />
          </FilterField>
        ...
        <View style={{ flexDirection: 'row', gap: 8 }}>
          <FilterField label="Location">
            <CompactDropdown value={location} options={LOCATION_LIST} onSelect={setLocation} />
          </FilterField>
          <FilterField label="Status">
            <CompactDropdown value={status} options={STATUS_LIST} onSelect={setStatus} />
          </FilterField>
          <FilterField label="Notice No">
            <CompactDropdown value={noticeNo} options={NOTICE_LIST} onSelect={setNoticeNo} />
          </FilterField>
        </View>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          ...
          >
            ...
            <Text style={{ fontSize: 13, fontWeight: '700', color: 'white' }}>Search</Text>
          </TouchableOpacity>
          ...
          <Checkbox value={checkAll} onToggle={toggleCheckAll} label="Check All" />
        </View>
      </View>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} persistentScrollbar>
          <View>
            ...
            <View style={{ flexDirection: 'row', borderBottomWidth: 2, borderColor: '#DBEAFE', backgroundColor: '#F0F5FF' }}>
              ...
              <View style={{ width: 54, paddingHorizontal: 10, paddingVertical: 10, borderRightWidth: 1, borderColor: '#DBEAFE', alignItems: 'center', justifyContent: 'center' }}>
                <Text style={{ fontSize: 10, fontWeight: '800', color: '#1D4ED8', textTransform: 'uppercase', letterSpacing: 0.6 }}>
                  Destroy
                </Text>
              </View>
              {tableCols.map(c => (
                <View
                  key={c.label}
                  style={{ width: c.width, paddingHorizontal: 12, paddingVertical: 10, borderRightWidth: 1, borderColor: '#DBEAFE', justifyContent: 'center' }}
                >
              ...
              renderItem={({ item, index }) => {
                ...
                return (
                  ...
                  >
                    ...
                    <View style={{ width: 54, alignItems: 'center', justifyContent: 'center', paddingVertical: 12, borderRightWidth: 1, borderColor: '#F1F5F9', paddingLeft: sel ? 6 : 0 }}>
                      <Checkbox value={destroyRows[index]} onToggle={() => toggleDestroy(index)} />
                    </View>
