  ...
  return (
    <View>
      <TouchableOpacity
        ...
        activeOpacity={0.7}
      >
        <Text className="flex-1 text-sm font-medium text-slate-900" numberOfLines={1}>{value}</Text>
    ...
    <SafeAreaView className="flex-1 bg-slate-50">
      ...
      <View className="flex-row items-center px-4 py-3 bg-white border-b border-slate-100">
        ...
        <View className="flex-1">
          <Text className="text-lg font-bold text-slate-900">Check In</Text>
        ...
        <View className="flex-row items-center bg-blue-50 border border-blue-200 rounded-full px-4 py-1.5">
          ...
          <Text className="text-[10px] font-bold text-blue-700" numberOfLines={1}>2F FG W/H</Text>
        </View>
      </View>

      {/* ── TOOLBAR ── */}
      <View className="bg-white border-b border-slate-200 px-4 pt-4 pb-4 shadow-sm">

        {/* Row 1: Shelf + Carton */}
        <View className="flex-row mb-3">
          <View className="flex-1 mr-3">
            <Text className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Shelf</Text>
            <CompactDropdown value={shelf} options={SHELVES} onSelect={setShelf} />
          </View>
          <View className="flex-1">
            <Text className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Carton No.</Text>
            <CompactDropdown value={carton} options={CARTONS} onSelect={setCarton} />
          </View>
        </View>
        ...
        <View className="flex-row mb-3">
          {/* No. stepper */}
          <View className="flex-1 flex-row items-center justify-between bg-slate-50 border border-slate-200 rounded-2xl px-3 h-11 mr-3">
            <Text className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">No.</Text>
            <View className="flex-row items-center gap-1">
              ...
              <Text className="text-sm font-bold text-slate-900 w-7 text-center">{no}</Text>
              <TouchableOpacity
                className="w-7 h-7 rounded-lg bg-blue-50 border border-blue-200 items-center justify-center"
                onPress={() => setNo(no + 1)}
                activeOpacity={0.7}
              >
                <Feather name="plus" size={13} color="#3B82F6" />
              </TouchableOpacity>
            </View>
          </View>

          {/* Punch Hole toggle */}
          <View className="flex-1 flex-row items-center justify-between bg-slate-50 border border-slate-200 rounded-2xl px-3 h-11">
            <Text className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Punch Hole</Text>
            <Switch
              value={punchHole}
        ...
        <View className="flex-row mb-2">
          <TouchableOpacity
            className={`flex-1 h-12 rounded-2xl flex-row justify-center items-center mr-2 ${scanning ? 'bg-red-500' : 'bg-blue-600'}`}
            onPress={() => setScanning(!scanning)}
            activeOpacity={0.8}
          >
            <Feather name={scanning ? 'square' : 'radio'} size={16} color="white" style={{ marginRight: 7 }} />
            <Text className="text-sm font-bold text-white tracking-wider">{scanning ? 'STOP' : 'SCAN'}</Text>
          </TouchableOpacity>

          <TouchableOpacity
            className="flex-1 h-12 bg-emerald-500 rounded-2xl flex-row justify-center items-center"
            activeOpacity={0.8}
          >
            <Feather name="save" size={16} color="white" style={{ marginRight: 7 }} />
        ...
        <View className="flex-row gap-2">
          {([
            { icon: 'refresh-cw', label: 'Refresh', bg: '#EFF6FF', border: '#BFDBFE', iconColor: '#2563EB', textColor: '#1D4ED8' },
            { icon: 'trash-2', label: 'Clear', bg: '#FEF2F2', border: '#FECACA', iconColor: '#DC2626', textColor: '#B91C1C' },
            { icon: 'crosshair', label: 'Location', bg: '#F0FDF4', border: '#BBF7D0', iconColor: '#16A34A', textColor: '#15803D' },
          ] as const).map(btn => (
            <TouchableOpacity
              key={btn.label}
              style={{ flex: 1, height: 36, backgroundColor: btn.bg, borderWidth: 1, borderColor: btn.border, borderRadius: 12, flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }}
              activeOpacity={0.7}
            >
              <Feather name={btn.icon} size={13} color={btn.iconColor} style={{ marginRight: 5 }} />
              <Text style={{ fontSize: 12, fontWeight: '600', color: btn.textColor }}>{btn.label}</Text>
            </TouchableOpacity>
          ))}
        </View>
      </View>
      ...
      <View className="flex-1 bg-white border-b-4 border-slate-200">
        ...
        <View className="flex-row items-center px-4 pt-2.5 pb-1.5 gap-2">
          <Text className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">History</Text>
          <View className="bg-blue-50 px-2 py-0.5 rounded-md">
            <Text className="text-[10px] font-bold text-blue-600">{HISTORY.length}</Text>
          </View>
        </View>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          <View>
            ...
            <View className="flex-row border-b-2 border-blue-100"
      ...
      <View className="flex-[1.2] bg-white">
        ...
        <View className="flex-row items-center px-4 pt-2.5 pb-1.5 gap-2">
          ...
          <View className={`flex-row items-center px-2 py-0.5 rounded-md gap-1 ${scanning ? 'bg-blue-50' : 'bg-slate-100'}`}>
            ...
            <Text className={`text-[10px] font-bold ${scanning ? 'text-blue-600' : 'text-slate-400'}`}>{TAGS.length}</Text>
          </View>
        </View>
